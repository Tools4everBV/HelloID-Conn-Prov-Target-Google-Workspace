############################################################################
# HelloID-Conn-Prov-Target-GoogleWorkSpace-Permissions-Drives-Grant
# PowerShell V2
############################################################################
    ...
    switch ($action) {
        'GrantPermission' {
            ...
            $addOrganizerSplatParams = @{
                Uri         = "https://www.googleapis.com/drive/v3/files/$($actionContext.References.Permission.Reference)/permissions?supportsAllDrives=true&useDomainAdminAccess=true&sendNotificationEmail=false"
                Headers     = $headers
                Method      = "POST"
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Granting GoogleWS drive permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                try {
                ...
                Write-Information "[DryRun] Grant GoogleWS drive permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
