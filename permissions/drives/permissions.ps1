#########################################################################
# HelloID-Conn-Prov-Target-GoogleWorkSpace-Permissions-Drives-Permissions
# PowerShell V2
#########################################################################
...
try {
    ...
    Write-Information 'Retrieve existing GoogleWS drives'
    $splatGetDrives = @{
        Uri     = "https://www.googleapis.com/drive/v3/drives?useDomainAdminAccess=true"
        Method  = 'GET'
        Headers = $headers
    }
    $retrievedPermissions = Invoke-GoogleWSRestMethodWithPaging @splatGetDrives -CollectionName 'drives'
