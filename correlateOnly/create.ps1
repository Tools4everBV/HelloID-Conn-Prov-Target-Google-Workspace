...
try {
    ...
    if ($actionContext.CorrelationConfiguration.Enabled) {
        ...
        # Determine if a user can be correlated
        try {
    ...
    elseif ($correlatedAccount.users.Count -gt 1) {
        $action = "MultipleFound"
    }
    elseif ($correlatedAccount.users.Count -eq 0) {
        $action = "NotFound"
    }
    ...
    switch ($action) {
        'CorrelateAccount' {
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                $auditLogMessage = "[DryRun] Correlate GoogleWS account: [$($correlatedAccount.users.id | ConvertTo-Json)] on [$($correlationField)] = [$($correlationValue)], will be executed during enforcement"
            }
            break
        }

        'MultipleFound' {
            throw "Multiple accounts found for person where [$($correlationField)] = [$($correlationValue)]"
        }

        'NotFound' {

            throw "No accounts found for person where [$($correlationField)] = [$($correlationValue)]"
        }
    }
